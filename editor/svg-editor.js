...
'          <defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
  ...
  const Actions = (function () {
    ...
    const toolButtons = [
      ...
      {sel: '#tool_circle', fn: clickCircle, evt: 'mouseup', parent: '#tools_rect', icon: 'circle'},
      {sel: '#tool_fhellipse', fn: clickFHEllipse, evt: 'mouseup', parent: '#tools_rect', icon: 'fh_ellipse'},
      {sel: '#tool_path', fn: clickPath, evt: 'click', key: ['P', true]},
      {sel: '#tool_text', fn: clickText, evt: 'click', key: ['T', true], parent: '#tools_rect', icon: 'text'},
  ...
  function handle (delta) {
    const zoom = svgCanvas.getZoom();
    changeZoom({value: (zoom + delta / 10.0) * 100});
    // // 向下滚动
    // if (delta < 0) {
    //   changeZoom({value: zoom - delta * 100});
    // } else {
    //   // 向上滚动
    //   changeZoom({value: delta * 100});
    // }
  }
  // Select given tool
  editor.ready(function () {
