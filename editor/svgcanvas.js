...
class SvgCanvas {
  constructor (container, config) {
    ...
    const getJsonFromSvgElement = this.getJsonFromSvgElement = function (data) {
      ...
      // Iterate attributes
      for (let i = 0, attr;
        (attr = data.attributes[i]); i++) {
        retval.attr[attr.name] = attr.value;
      }

      // Iterate children
      for (let i = 0, node;
        (node = data.childNodes[i]); i++) {
...
setTimeout(function () {
    ...
    this.setDocumentTitle = function (newtitle) {
      ...
      batchCmd.addSubCommand(new ChangeElementCommand(docTitle, {
        '#text': oldTitle
      }));
      addCommandToHistory(batchCmd);
    };
    ...
    this.copySelectedElements = function () {
      localStorage.setItem('svgedit_clipboard', JSON.stringify(
        selectedElements.map(function (x) {
